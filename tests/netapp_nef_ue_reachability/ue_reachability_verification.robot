...
Create subscription by Authorized Network Application
    ...
    ${resp}=            Run Keyword   Connection Monitoring Ue Reachability  ${NEF_HOSTNAME}  ${CERTIFICATE_FOLDER}  ${CAPIF_HOST}  ${CAPIF_PORT}  ${NEF_CALLBACK_HOSTNAME}
...
Create subscription when there is already an active subscription for a registered UE
    ...
    ${resp}=            Run Keyword And Expect Error  *    Connection Monitoring Ue Reachability  ${NEF_HOSTNAME}  ${CERTIFICATE_FOLDER}  ${CAPIF_HOST}  ${CAPIF_PORT}  ${NEF_CALLBACK_HOSTNAME}
